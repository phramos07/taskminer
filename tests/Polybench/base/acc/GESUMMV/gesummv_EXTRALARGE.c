...
void gesummv(DATA_TYPE * A, DATA_TYPE * B,
...
void GPU__gesummv(DATA_TYPE * A, DATA_TYPE * B,
                  DATA_TYPE * x, DATA_TYPE * y,
                  DATA_TYPE * tmp) {
  int i, j;

  char RST_AI1 = 0;
  RST_AI1 |= !((A + 0 > B + 16777216)
  || (B + 0 > A + 16777216));
  ...
  RST_AI1 |= !((x + 0 > y + 4096)
  || (y + 0 > x + 4096));
  #pragma acc data pcopyin(A[0:16777216],B[0:16777216],x[0:4096]) pcopy(tmp[0:4096],y[0:4096]) if(!RST_AI1)
...
void compareResults(DATA_TYPE * y,
