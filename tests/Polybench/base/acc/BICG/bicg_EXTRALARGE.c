...
void init_array(DATA_TYPE * A, DATA_TYPE * p,
                DATA_TYPE * r) {
  int i, j;
...
void GPU__bicg(DATA_TYPE * A, DATA_TYPE * r,
               DATA_TYPE * s, DATA_TYPE * p,
               DATA_TYPE * q) {
  int i, j;

  #pragma acc data pcopy(s[0:8192])
  #pragma acc kernels
  for (i = 0; i < NY; i++) {
    s[i] = 0.0;
  }
  ...
  RST_AI2 |= !((r + 0 > s + 8192)
  || (s + 0 > r + 8192));
  #pragma acc data pcopyin(A[0:67108864],r[0:8192]) pcopy(s[0:8192]) if(!RST_AI2)
  ...
  RST_AI3 |= !((p + 0 > q + 8192)
  || (q + 0 > p + 8192));
  #pragma acc data pcopyin(A[0:67108864],p[0:8192]) pcopy(q[0:8192]) if(!RST_AI3)
