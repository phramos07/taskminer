...
void GPU__bicg(DATA_TYPE *restrict A, DATA_TYPE *restrict r,
  ...
  #pragma acc data pcopy(s[0:4096])
  #pragma acc kernels
  for (i = 0; i < NY; i++) {
    s[i] = 0.0;
  }
  ...
  RST_AI2 |= !((r + 0 > s + 4096)
  || (s + 0 > r + 4096));
  #pragma acc data pcopyin(A[0:16777216],r[0:4096]) pcopy(s[0:4096]) if(!RST_AI2)
  ...
  RST_AI3 |= !((p + 0 > q + 4096)
  || (q + 0 > p + 4096));
  #pragma acc data pcopyin(A[0:16777216],p[0:4096]) pcopy(q[0:4096]) if(!RST_AI3)
