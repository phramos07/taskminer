...
void init_array(DATA_TYPE * A, DATA_TYPE * x1,
                DATA_TYPE * x2, DATA_TYPE * y1,
                DATA_TYPE * y2, DATA_TYPE * x1_gpu,
                DATA_TYPE * x2_gpu) {
  int i, j;
...
void runMvt(DATA_TYPE * a, DATA_TYPE * x1,
            DATA_TYPE * x2, DATA_TYPE * y1,
            DATA_TYPE * y2) {
  int i, j;
...
void GPU__runMvt(DATA_TYPE * a, DATA_TYPE * x1,
                 DATA_TYPE * x2, DATA_TYPE * y1,
                 DATA_TYPE * y2) {
  int i;
...
// Note that you must collapse only outer loop to avoid conflicts
  ...
  || (y1 + 0 > x1 + 4096));
  #pragma acc data pcopyin(a[0:16777216],y1[0:4096]) pcopy(x1[0:4096]) if(!RST_AI1)
  ...
  RST_AI2 |= !((a + 0 > y2 + 4096)
  || (y2 + 0 > a + 16777216));
  RST_AI2 |= !((x2 + 0 > y2 + 4096)
  || (y2 + 0 > x2 + 4096));
  #pragma acc data pcopyin(a[0:16777216],y2[0:4096]) pcopy(x2[0:4096]) if(!RST_AI2)
...
void compareResults(DATA_TYPE * x1,
