...
void GPU__atax(DATA_TYPE *restrict A, DATA_TYPE *restrict x,
  ...
  #pragma acc data pcopy(y[0:4096])
  #pragma acc kernels
  for (i = 0; i < NY; i++) {
    y[i] = 0;
  }
  ...
  || (x + 0 > tmp + 4096));
  #pragma acc data pcopyin(A[0:16777216],x[0:4096]) pcopy(tmp[0:4096]) if(!RST_AI2)
...
// Note that the Loop has been reversed
  ...
  || (y + 0 > tmp + 4096));
  #pragma acc data pcopyin(A[0:16777216],tmp[0:4096]) pcopy(y[0:4096]) if(!RST_AI3)
