...
void GPU__atax(DATA_TYPE *restrict A, DATA_TYPE *restrict x,
  ...
  #pragma acc data pcopy(y[0:1024])
  #pragma acc kernels
  for (i = 0; i < NY; i++) {
    y[i] = 0;
  }
  ...
  || (x + 0 > tmp + 1024));
  #pragma acc data pcopyin(A[0:1048576],x[0:1024]) pcopy(tmp[0:1024]) if(!RST_AI2)
...
// Note that the Loop has been reversed
  ...
  || (y + 0 > tmp + 1024));
  #pragma acc data pcopyin(A[0:1048576],tmp[0:1024]) pcopy(y[0:1024]) if(!RST_AI3)
