...
void compareResults(DATA_TYPE * z,
                    DATA_TYPE * z_outputFromGpu) {
  int i, fail;
  fail = 0;
...
void GPU__atax(DATA_TYPE * A, DATA_TYPE * x,
               DATA_TYPE * y, DATA_TYPE * tmp) {
  int i, j;

  #pragma acc data pcopy(y[0:8192])
  #pragma acc kernels
  for (i = 0; i < NY; i++) {
    y[i] = 0;
  }
  ...
  char RST_AI2 = 0;
  RST_AI2 |= !((A + 0 > tmp + 8192)
  || (tmp + 0 > A + 67108864));
  RST_AI2 |= !((A + 0 > x + 8192)
  || (x + 0 > A + 67108864));
  RST_AI2 |= !((tmp + 0 > x + 8192)
  || (x + 0 > tmp + 8192));
  #pragma acc data pcopyin(A[0:67108864],x[0:8192]) pcopy(tmp[0:8192]) if(!RST_AI2)
...
// Note that the Loop has been reversed
  char RST_AI3 = 0;
  RST_AI3 |= !((A + 0 > tmp + 8192)
  || (tmp + 0 > A + 67108864));
  RST_AI3 |= !((A + 0 > y + 8192)
  || (y + 0 > A + 67108864));
  RST_AI3 |= !((tmp + 0 > y + 8192)
  || (y + 0 > tmp + 8192));
  #pragma acc data pcopyin(A[0:67108864],tmp[0:8192]) pcopy(y[0:8192]) if(!RST_AI3)
