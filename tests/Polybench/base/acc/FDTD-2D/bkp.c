...
void dump_arrays(DATA_TYPE *a, DATA_TYPE *b) {
  int i, j;
  
  fprintf(stderr, "\n\n---------- FIRST --------\n\n");
  for (i = 0; i < NX; i++) {
    for (j = 0; j < NY; j++) {
      fprintf(stderr, "%f\t", a[i*NY+j]);
    }
    fprintf(stderr, "\n");
  }
...
void runFdtd(DATA_TYPE * _fict_, DATA_TYPE * ex,
             DATA_TYPE * ey, DATA_TYPE * hz) {
  int t, i, j;
...
void GPU__runFdtd(DATA_TYPE * _fict_, DATA_TYPE * ex,
                  DATA_TYPE * ey, DATA_TYPE * hz) {
  int t, i, j;
...
int main() {
  ...
  dump_arrays(hz, hz_outputFromGpu);
