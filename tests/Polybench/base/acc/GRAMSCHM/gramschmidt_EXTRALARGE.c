...
void gramschmidt(DATA_TYPE * A, DATA_TYPE * R,
                 DATA_TYPE * Q) {
  int i, j, k;
  DATA_TYPE nrm;
  for (k = 0; k < N; k++) {
...
void GPU__gramschmidt(DATA_TYPE * A, DATA_TYPE * R,
                      DATA_TYPE * Q) {
  int i, j, k;
  DATA_TYPE nrm;
  ...
  || (R + 0 > Q + 1048576));
  #pragma acc data pcopy(A[0:1048576],Q[0:1048576],R[0:1048576]) if(!RST_AI1)
...
void compareResults(DATA_TYPE * A,
