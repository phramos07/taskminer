...
void GPU__bicg(DATA_TYPE *restrict A, DATA_TYPE *restrict r,
  ...
  #pragma omp target data map(tofrom: s[0:256])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    s[i] = 0.0;
  }
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI2 |= !((r + 0 > s + 256)
  || (s + 0 > r + 256));
  #pragma omp target data map(to: A[0:65536],r[0:256]) map(tofrom: s[0:256]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI3 |= !((p + 0 > q + 256)
  || (q + 0 > p + 256));
  #pragma omp target data map(to: A[0:65536],p[0:256]) map(tofrom: q[0:256]) if(!RST_AI3)
