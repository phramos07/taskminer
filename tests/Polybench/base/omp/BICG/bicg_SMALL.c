...
void GPU__bicg(DATA_TYPE *restrict A, DATA_TYPE *restrict r,
  ...
  #pragma omp target data map(tofrom: s[0:512])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    s[i] = 0.0;
  }
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI2 |= !((r + 0 > s + 512)
  || (s + 0 > r + 512));
  #pragma omp target data map(to: A[0:262144],r[0:512]) map(tofrom: s[0:512]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI3 |= !((p + 0 > q + 512)
  || (q + 0 > p + 512));
  #pragma omp target data map(to: A[0:262144],p[0:512]) map(tofrom: q[0:512]) if(!RST_AI3)
