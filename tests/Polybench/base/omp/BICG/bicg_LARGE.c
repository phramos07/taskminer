...
void GPU__bicg(DATA_TYPE *restrict A, DATA_TYPE *restrict r,
  ...
  #pragma omp target data map(tofrom: s[0:4096])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    s[i] = 0.0;
  }
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI2 |= !((r + 0 > s + 4096)
  || (s + 0 > r + 4096));
  #pragma omp target data map(to: A[0:16777216],r[0:4096]) map(tofrom: s[0:4096]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI3 |= !((p + 0 > q + 4096)
  || (q + 0 > p + 4096));
  #pragma omp target data map(to: A[0:16777216],p[0:4096]) map(tofrom: q[0:4096]) if(!RST_AI3)
