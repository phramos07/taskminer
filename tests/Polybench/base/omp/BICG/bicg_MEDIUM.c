...
void GPU__bicg(DATA_TYPE *restrict A, DATA_TYPE *restrict r,
  ...
  #pragma omp target data map(tofrom: s[0:1024])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    s[i] = 0.0;
  }
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI2 |= !((r + 0 > s + 1024)
  || (s + 0 > r + 1024));
  #pragma omp target data map(to: A[0:1048576],r[0:1024]) map(tofrom: s[0:1024]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI3 |= !((p + 0 > q + 1024)
  || (q + 0 > p + 1024));
  #pragma omp target data map(to: A[0:1048576],p[0:1024]) map(tofrom: q[0:1024]) if(!RST_AI3)
