...
void GPU__atax(DATA_TYPE *restrict A, DATA_TYPE *restrict x,
  ...
  #pragma omp target data map(tofrom: y[0:256])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    y[i] = 0;
  }
...
#pragma omp parallel for
  ...
  RST_AI2 |= !((tmp + 0 > x + 256)
  || (x + 0 > tmp + 256));
  #pragma omp target data map(to: A[0:65536],x[0:256]) map(tofrom: tmp[0:256]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI3 |= !((tmp + 0 > y + 256)
  || (y + 0 > tmp + 256));
  #pragma omp target data map(to: A[0:65536],tmp[0:256]) map(tofrom: y[0:256]) if(!RST_AI3)
