...
void GPU__atax(DATA_TYPE *restrict A, DATA_TYPE *restrict x,
  ...
  #pragma omp target data map(tofrom: y[0:4096])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    y[i] = 0;
  }
...
#pragma omp parallel for
  char RST_AI2 = 0;
  RST_AI2 |= !((A + 0 > tmp + 4096)
  || (tmp + 0 > A + 16777216));
  RST_AI2 |= !((A + 0 > x + 4096)
  || (x + 0 > A + 16777216));
  RST_AI2 |= !((tmp + 0 > x + 4096)
  || (x + 0 > tmp + 4096));
  #pragma omp target data map(to: A[0:16777216],x[0:4096]) map(tofrom: tmp[0:4096]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  char RST_AI3 = 0;
  RST_AI3 |= !((A + 0 > tmp + 4096)
  || (tmp + 0 > A + 16777216));
  RST_AI3 |= !((A + 0 > y + 4096)
  || (y + 0 > A + 16777216));
  RST_AI3 |= !((tmp + 0 > y + 4096)
  || (y + 0 > tmp + 4096));
  #pragma omp target data map(to: A[0:16777216],tmp[0:4096]) map(tofrom: y[0:4096]) if(!RST_AI3)
