...
void GPU__atax(DATA_TYPE *restrict A, DATA_TYPE *restrict x,
  ...
  #pragma omp target data map(tofrom: y[0:8192])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    y[i] = 0;
  }
...
#pragma omp parallel for
  ...
  || (x + 0 > tmp + 8192));
  #pragma omp target data map(to: A[0:67108864],x[0:8192]) map(tofrom: tmp[0:8192]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  ...
  || (y + 0 > tmp + 8192));
  #pragma omp target data map(to: A[0:67108864],tmp[0:8192]) map(tofrom: y[0:8192]) if(!RST_AI3)
