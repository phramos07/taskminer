...
void GPU__atax(DATA_TYPE *restrict A, DATA_TYPE *restrict x,
  ...
  #pragma omp target data map(tofrom: y[0:512])
  #pragma omp target
  for (i = 0; i < NY; i++) {
    y[i] = 0;
  }
...
#pragma omp parallel for
  ...
  RST_AI2 |= !((tmp + 0 > x + 512)
  || (x + 0 > tmp + 512));
  #pragma omp target data map(to: A[0:262144],x[0:512]) map(tofrom: tmp[0:512]) if(!RST_AI2)
...
#pragma omp parallel for collapse(1)
  ...
  RST_AI3 |= !((tmp + 0 > y + 512)
  || (y + 0 > tmp + 512));
  #pragma omp target data map(to: A[0:262144],tmp[0:512]) map(tofrom: y[0:512]) if(!RST_AI3)
