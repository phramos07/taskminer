...
{
	int maxLargura, x, y, peso, pesototal=0, ans;
	unsigned numPontos;
	scanf("%d", &maxLargura);
	scanf("%d", &numPontos);
	int debug = 0;
	...
	while (numPontos > 0)
	{
		...
		P.x = x;
		P.y = y;
		ans = addNovaEstrelaNoPontoX(P, peso, A);
		if (ans)
		{
			#pragma omp critical
			pesototal += peso;
...
int addNovaEstrelaNoPontoX(Ponto P, int peso, ArvoreQuad* A)
{
	...
	int dentro = estrelaDentroDoQuadrante(nova, A->raiz);
	if (dentro)
	{
		#pragma omp task
...
void addEstrelaAoQuadranteRecursivamente(Quadrante* Q, Estrela* star)
{
	...
	int dentro;
	#pragma omp critical
	dentro = estrelaDentroDoQuadrante(star, Q);
	if (!dentro)
		return;
