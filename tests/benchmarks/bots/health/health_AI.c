...
int check_village(struct Village *top)
{
   ...
   if (res_population != result.total_patients) answer = -1;
   if (res_hospitals != result.hosps_number) answer = -1;
   if (res_personnel != result.hosps_personnel) answer = -1;
   if (res_checkin != result.total_hosps_v) answer = -1;
   if (res_village != result.total_in_village) answer = -1;
   if (res_waiting != result.total_waiting) answer = -1;
   if (res_assess != result.total_assess) answer = -1;
   if (res_inside != result.total_inside) answer = -1;
