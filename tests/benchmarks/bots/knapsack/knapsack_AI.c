...
void knapsack_par(struct item *e, int c, int n, int v, int *sol, int l) {
  ...
  #pragma omp task untied default(shared) depend(in:e[1]) depend(out:without) firstprivate(without,e[1]) if(cutoff_test)
  knapsack_par(e + 1, c, n - 1, v, &without, l + 1);

  /* compute the best solution with the current item in the knapsack */
  cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
  #pragma omp task untied default(shared) depend(in:e[1]) depend(out:with) firstprivate(with,e[1]) if(cutoff_test)
