...
void init_arrays(float *A, float *B) {
  ...
  for (i = 0; i < SIZE; ++i) {
    {
    int tm_cost1 = (21);
    #pragma omp task depend(inout: A[0:1001],B[0:1001]) if(tm_cost1 > 41)
    {
    A[i] = (float)i + 3 * i;
    B[i] = (float)i + 2 * i;
  }
...
void product_GPU(float *A, float *B, float *C) {
...

  {
    ...
    for (i = 0; i < SIZE; ++i) {
      {
      int tm_cost1 = (17);
      #pragma omp task depend(inout: A[0:1001],B[0:1001],C[0:1001]) if(tm_cost1 > 41)
      {
      C[i] = A[i] * B[i];
    }
    }
...
void compareResults(float *A, float *A_outputFromGpu) {
  ...
  for (i = 0; i < SIZE; i++) {
    {
    int tm_cost1 = (24);
    #pragma omp task depend(inout: A[0:1001],A_outputFromGpu[0:1001]) if(tm_cost1 > 41)
    {
