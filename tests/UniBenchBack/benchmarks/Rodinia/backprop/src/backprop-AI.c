...
int m, n;
{
  ...
  AI1[2] = 8 * AI1[1];
  AI1[3] = AI1[2] / 4;
  #pragma acc data copy(new[0:AI1[3]])
  #pragma acc kernels
...
void compareResults(float *l2, float *l2_gpu, int n2) {
  ...
  AI1[3] = 4 + AI1[2];
  AI1[4] = AI1[3] / 4;
  #pragma acc data copy(l2[1:AI1[4]],l2_gpu[1:AI1[4]])
  #pragma acc kernels
  #pragma acc loop independent
  for (i=1; i<=n2; i++) {
...
int n1, n2;
{
  ...
  #pragma omp target map(to: conn_gpu[:(n1+1)*(n2+1)], l1[:n1+1]) map(tofrom: l2_gpu[:n2+1])
  {
	  ...
	  AI1[14] = 4 + AI1[13];
	  AI1[15] = AI1[14] / 4;
	  #pragma acc data copy(l1[0:AI1[4]],conn_gpu[1:AI1[12]],l2_gpu[1:AI1[15]])
	  #pragma acc kernels
	  #pragma acc loop independent
	  for (j = 1; j <= n2; j++) {
...
void test(float *delta_h, int nh) {
  ...
  AI1[2] = 4 * AI1[1];
  AI1[3] = AI1[2] / 4;
  #pragma acc data copy(delta_h[0:AI1[3]])
  #pragma acc kernels
  #pragma acc loop independent
  for (j = 0; j < nh; j++)
    delta_h[j] = j + 1.0f;
...
float *delta, *ly, **w, **oldw;
{
  ...
  #pragma omp target map(to: ly[:(nly+1)], delta[:(ndelta+1)]) map(tofrom: oldw_gpu[:size], w_gpu[:size])
  {
	  ...
	  AI1[10] = 1 + AI1[1];
	  AI1[11] = ndelta * AI1[7];
	  AI1[12] = AI1[10] + AI1[11];
	  AI1[13] = 4 * AI1[12];
	  AI1[14] = AI1[13] * 1;
	  AI1[15] = AI1[14] / 4;
	  #pragma acc data copy(delta[1:AI1[4]],ly[0:AI1[9]],w_gpu[1:AI1[15]],oldw_gpu[1:AI1[15]])
	  #pragma acc kernels
	  #pragma acc loop independent
	  for (j = 1; j <= ndelta; j++) {
	    for (k = 0; k <= nly; k++) {
