...
\label{def:hammock}
A hammock region $G'$ from a CFG $G$ is a subgraph $G' \subseteq G$, with a
node $h$, that {\em dominates}\footnote{A node $n_1 \in G$ dominates another node
...
In this paper, {\em we extract the smallest thread above the runtime cost}.
To bestow meaning into this last statement, in what follows, we define more
formally the notions of task size and task expansion.
...
only be created if the expression \textsf{5 $\times$ M $<$ WORK\_CUTOFF}
holds.
...
that is contained entirely within a windmill $W$.
...
$H_1$ is a child of $H_2$ in this tree if $H_1 \subset H_2$, and for any other
node $H_x$, $H_1 \subset H_x \Rightarrow H_2 \subset H_x$.
In this context, we call $H_2$ the \textsf{PARENT} of $H_1$.
