...
\paragraph{Rational:}
We expand tasks up to a certain threshold \textsf{COST}, which is determined
by the cost model mentioned in Section~\ref{sub:profit}.
If the profitability of a task is less than this constant, then parallelism
might not pay off the cost of managing threads.
However, if we expand tasks too much, then we might not have much parallelism,
because code within a task is executed sequentially.
Thus, determining \textsf{COST} is essential to the performance of our
optimization.
In this work, we have determined this parameter empirically for the
