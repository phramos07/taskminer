...
replicated in new tasks, or shared among them (Section~\ref{sub:variance}).
...
techniques that we discuss in Section~\ref{sec:sol}.
In the rest of this section, we discuss them.

\subsection{The Advantages}
\label{sub:adv}

The main advantage of using OpenMP to split a program into parallel tasks is
its runtime.
This runtime lets us circumvent the shortcomings that pointer aliasing
...
resort to complex runtime checks to identify it
dynamically~\cite{Alves15,Rus02}.
The OpenMP runtime shields us from having to implement such checks, because
it already checks for dependencies among tasks, and dispatches them in a
correct order~\cite{LaGrone11}.
An OpenMP task descriptor contains the task's status (running, waiting) and a
list of memory addresses the task registered as input arguments.

The process of checking for dependencies can be as simple as checking if an
incoming task in the system accesses a memory address already in use by a
previous task in flight.
...
Tasks are dispatched for execution according to a topological ordering of this
graph~\cite{Planas15}.

This runtime support allows the parallelization of irregular applications,
such as programs that traverse data-structures formed by a mesh of pointers.
As an example, Figure~\ref{fig:book_filter} shows an application that
...
Figure~\ref{fig:book_filter} has been annotated automatically by the \Taskminer.
We emphasize that neither the runtime checks of Alves
{\em et al.}~\cite{Alves15} or Rus {\em et al.}~\cite{Rus02}, nor a
context and flow sensitive alias analysis, \`{a} la Whaley and
Lam~\cite{Whaley04} would be able to ensure the automatic parallelization of
this program.
