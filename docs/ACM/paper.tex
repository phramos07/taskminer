...
The {\Taskminer} algorithm is illustrated in FIgure \ref{fig:alg_main}. 
It uses information from both the \emph{Control Flow Graph} and
the \emph{Dependence Graph}. It relies on the concepts of
\emph{windmills} and \emph{helices} (Definitions \ref{def:helix} and \ref{def:windmill}).

\begin{definition}[Windmill]
\label{def:windmill}
Given a set $S$ of strongly connected nodes in the program \emph{Dependence Graph} $G$, 
a windmill $W$ is defined as:
$$W = \bigcup_i N_i[S]$$
\end{definition}
...
%I don't know much of this part. I asked gleison to write me a paragraph and
%then I'll review it and try to write something here.

\subsection{Estimating the Profitability of Tasks}
\label{sub:profit}

%We need to discuss the THRESHOLD. Are we going to use
...
%I can work on that on the weekend!
...
%I asked gleison and he'll sit with me to explain everything ,
