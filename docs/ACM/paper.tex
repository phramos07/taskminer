...
to very trivial annotations, which would hardly be of any use.
...
For reasons that will be made more clear in Section~\ref{sub:identification},
...
Figure~\ref{fig:ex_Regions} also lets us introduce the second challenge that we
...
The creation of tasks involves a non-negligible runtime cost, which includes
the allocation of resources, and the scheduling of the task proper.
Ideally, this cost should be paid only when the task performs an amount of
work that is sufficiently large to pay for its management cost.
...
However, we can try to approximate this quantity, using, to this end, program
symbols, which are replaced with actual values at runtime.
...
To introduce Challenge~\ref{ch:cutoff}, we quote Duran {\em et al.}:
...
counter -- \textsf{taskminer\_depth\_cutoff} in Figure~\ref{fig:cutoff}.
The guard in line 7 ensures that we never exceed \textsf{DEPTH\_CUTOFF}, a
predetermined threshold.
The code generation algorithms that we discuss in this paper are
...
Challenges~\ref{ch:Regions}, and~\ref{ch:privatize}, on the other hand, are
...
Because \textsf{j} is writte within these tasks, race conditions would
ensue.
In Section~\ref{sub:variance} we explain how we separate private from
shared variables.
