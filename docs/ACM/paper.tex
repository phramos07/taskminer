...
\paragraph{The syntax that we handle.}
...
\label{def:task}
Given a program $P$, a task $T$ is a tuple $(G', M)$ formed by a hammock region
$G'$, plus a set $M$ of memory regions that represent the dependences of $T$.
\end{definition}
...
If a task $(T_1, M_1)$ writes onto a memory region $m$, and task
$(T_2, M_2)$ contains code that reads $m$, then $m \in M_1$, to ensure that
dependences are respected.
If a program $P$ is partitioned into a set of $n$ tasks $\{(T_1, M_1),
\ldots (T_n, M_n)\}$, then this partition is said to be {\em correct} if any
topological ordering of the tasks, as determined by dependence relations, always
leads to the same results as the sequential execution of $P$.

\paragraph{The annotations that we use.}
