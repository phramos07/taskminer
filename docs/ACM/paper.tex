...
plus ranges of offsets that these pointers are allowed to dereference.
...
\label{ex:regions}
\fer{XX Add an example to describe the notions of regions and dependences.
The best is to modify one of the programs in Section 2, so that we can refer to
that program.
Pedro, can you think about it and propose something?}
\end{example}
...
finding potential tasks relies on a concept that we call \emph{Helices}.

\begin{definition}[Helix]
\label{def:helix}
Given a set $S$ of strongly connected nodes in the Dependence Graph $G$, a set of nodes $H$ is a Helix if, and only if:
\begin{itemize}
\item $H \subset \bigcup_i N_i[S]$;
\item $H  \cap S = \emptyset$;
\item $H$ is connected and maximal.
\end{itemize}
\end{definition}

We can view a \emph{Helix} as a set of nodes in the PDG that depends on a Strongly Connected Component
(SCC) but \emph{do not integrate the SCC}. That means helices are all those subgraphs that depend on
the SCC and do not form a cyclic dependence with the SCC itself.

Statically, a Strongly Connected Component in the PDG represents iteration dependence, so it
arises due to loops or recursive patterns, should the PDG is interprocedural. The insight behind 
\emph{Helices} is that they represent instructions that are deemed to be executed more than once
for they depend on a cyclic dependence (the SCC), and, since they do not integrate the SCC, they can
